...
<div class="grid-item <?php echo esc_attr( $args['classes'] ); ?>">
			...
			if ( ! empty( $args['title'] ) ) {
				...
				<div class="grid-item-header">
					<?php if ( ! empty( $args['icon_file'] ) ) { ?>
						<img class="grid-item-icon" src="<?php echo esc_url( THEME_IMAGES . 'icons/' . $args['icon_file'] . '.svg' ); ?>" alt="">
					<?php } ?>
					<span class="grid-item-title h4"><?php echo esc_html( $args['title'] ); ?></span>
				</div>
				<?php
			}
			...
			if ( ! empty( $args['button_url'] ) ) {
				...
				<div class="cta">
					...
					crispydiv_button( array(
						...
						'alt_target_self'      => $args['alt_link_target_self'],
					) );
					?>
				</div>
				<?php
